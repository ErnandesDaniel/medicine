...
function createData(id, patientName, patientPolicy, email, address,date_of_birth,  uziDate, hasNodules, isActive) {
    return {id: id, patientName: patientName,patientPolicy: patientPolicy, email: email, address: address, 
        date_of_birth: date_of_birth.toLocaleDateString(), uziDate: uziDate.toLocaleDateString(),hasNodules: hasNodules,isActive: isActive};
}
...
const MyGrid = (props) => {
    ...
    {
        name: 'Дата рождения', width: '160px', sortable: false, selector: row => row.date_of_birth,
    },
    {
        name: 'Адрес', width: '160px', sortable: false, selector: row => row.address,
    },
    ...
    useEffect(() => {
        ...
        axios.get(props.url + '/med_worker/patients/'+localStorage.getItem('id')).then((response) => {
            ...
            for (let cur of response.data.results.cards) {
                console.log(cur)
                tmpAr.push(createData(cur.patient.id, cur.patient.last_name + " " + cur.patient.first_name + " " + cur.patient.fathers_name, cur.patient.personal_policy, cur.patient.email, cur.patient.address,
                 new Date(cur.patient.date_of_birth),  
                 new Date(Date.parse(cur.acceptance_datetime)), cur.has_nodules === "T", cur.patient.is_active))
            }
        })
